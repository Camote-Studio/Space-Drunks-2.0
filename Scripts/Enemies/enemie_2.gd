...
var random_move_chance := 0.1
var random_move_duration := 0.8

# ======= KNOCKBACK (nuevo) =======
@export var knockback_resistance: float = 0.3   # 0 = recibe todo el empuje, 1 = lo ignora
var _knockback_velocity: Vector2 = Vector2.ZERO
var _knockback_timer: Timer = null

func apply_knockback(force: Vector2, duration: float = 0.25) -> void:
	if dead:
		return
	_knockback_velocity = force * (1.0 - clamp(knockback_resistance, 0.0, 1.0))
	if _knockback_timer == null:
		_knockback_timer = Timer.new()
		_knockback_timer.one_shot = true
		add_child(_knockback_timer)
		_knockback_timer.connect("timeout", Callable(self, "_end_knockback"))
...
func _ready() -> void:
	...
	# Timer para movimiento aleatorio
...
func _physics_process(delta: float) -> void:
	...
	if player != null:
		...
		# APLICAR KNOCKBACK ENCIMA DEL MOVIMIENTO
		velocity += _knockback_velocity

		move_and_slide()

		# Intentar atacar (usa dist y dir calculados arriba)
	...
	# Si no hay jugador -> patrulla
		...
		if random_move_dir == Vector2.ZERO and not random_move_timer.is_stopped():
			...
			if random_move_dir == Vector2.ZERO:
				_start_random_move()
			velocity = random_move_dir * (speed * 0.4)

		# aplicar knockback si lo hay
		velocity += _knockback_velocity
		move_and_slide()
